...
Today, this behavior is not possible because the feedback provider will only act if there was an error in execution.
To support this, the feedback subsystem would need to be extended so it may be called when no error occurred,
see [issue19372](https://github.com/PowerShell/PowerShell/issues/19372).
...
We don't currently do that for `out-default` which is also added to the pipeline.
I believe that history is what the user _typed_,
so we should extend the history object to include the _executed_ pipeline as an additional property.
